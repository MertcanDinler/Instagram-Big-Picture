...
class DatabaseHelper {
  ...
  void _onCreate(Database db, int ver) async {
    await db.execute(
        "CREATE TABLE Users(id INTEGER PRIMARY KEY, username TEXT, full_name TEXT, small TEXT, last_search INTEGER)");
  }
  ...
  Future<int> deleteUser(User user) async {
    var dbClient = await db;
    int resp =
        await dbClient.delete("Users", where: "id = ?", whereArgs: [user.id]);
    return resp;
  }
  ...
  Future<int> updateUser(User user) async {
    ...
    temp.remove("hd");
    temp["last_search"] = new DateTime.now().millisecondsSinceEpoch;
    int resp = await dbClient
        .update("Users", temp, where: 'id = ?', whereArgs: [user.id]);
    return resp;
  }
  ...
  Future<List<User>> getRecentlyUsers(int lim) async {
    var dbClient = await db;
    List<Map> maps =
        await dbClient.query("Users", orderBy: "last_search DESC", limit: lim);
